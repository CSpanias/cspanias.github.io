...
_For the purpose of performing permission checks, traditional UNIX implementations distinguish two categories of processes:_
1. _privileged processes (whose effective user ID is 0, referred to as superuser or root)_ 
2. _unprivileged processes (whose effective UID is nonzero)_

_Privileged processes bypass all kernel permission checks, while unprivileged processes are subject to full permission checking based on the process's credentials (usually: effective UID, effective GID, and supplementary group list)._
