...
The default command, that is without any port specification, scans the **most common 1,000 ports for each protocol** ([documentation](scans the most common 1,000 ports for each protocol)), so we can try scanning for all ports by adding the `-p` option:
