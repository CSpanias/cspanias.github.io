...
date: 2023-12-04
categories: [CTF, HTB]
tags: [nmap, cve-2023-27163, ssrf, command-injection]
img_path: /assets/sau/
published: true
---
...
cat ~/user.txt
b07c7d68947d6bf0f260971bd92d0f46
```
...
```shell
# find where trail.service is located
puma@sau:/opt/maltrail$ find / -iname trail.service -type f 2>/dev/null
find / -iname trail.service -type f 2>/dev/null
/etc/systemd/system/trail.service
```

```shell
# check file type of trail.service
puma@sau:/etc/systemd/system$ file trail.service
file trail.service
trail.service: Generic INItialization configuration [Service]\015
# check train.service content
...
puma@sau:/opt/maltrail$ ls -l /etc/systemd/system/trail.service
ls -l /etc/systemd/system/trail.service
-rwxr-xr-x 1 root root 461 Apr 15  2023 /etc/systemd/system/trail.service
```

```shell
# check perms for server.py
puma@sau:/opt/maltrail$ ls -l server.py
ls -l server.py
...
# cat /root/root.txt
cat /root/root.txt
0898964fe04d41a0680d6c8634108517
```
